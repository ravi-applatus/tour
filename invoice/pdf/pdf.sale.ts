...
export const generateSalePDF = (invoice: InvoiceEntity) => {
  // Create a document
  const filePath = `/hotel_voucher/invoice_sale_hotel_${uuid()}.pdf`;
  ...
  //////////////////////////////////////////////////////
  const transferAmount = invoice.transferAmount;
  const tourAmount = invoice.orderTours.reduce(
    (sum, orderTour) => sum + orderTour.amount,
    0,
  );
  const tourCommission = invoice.orderTours.reduce(
    (sum, orderTour) => sum + orderTour.commission,
    0,
  );
  //////////////////////////////////////////////////////
  ...
  invoice.orderHotels.forEach((orderHotel, orderIndex) => {
    orderHotel.orderHotelMapRooms.forEach((roomMap, mapIndex) => {
      ...
      pdf.addText(
        `${(
          Math.round((roomMap.amount / roomMap.quantity / nights) * 100) / 100
        ).toFixed(2)} USD`,
        85,
        523,
        150,
      ...
      pdf.addText(`${roomMap.amount} USD`, 285, 523, 150, {
        size: 10,
      });
      ...
      pdf.addText(`Tour Commission`, 190, 600, 200, {
        size: 10,
      });
      pdf.addText(`-${tourCommission} USD`, 285, 600, 150, {
        size: 10,
      });

      pdf.doc.moveTo(280, 618).lineTo(330, 618).stroke(); // create line

      pdf.addText('Grand Total:', 190, 630, 200, { size: 10 });
      pdf.addText(`${invoice.totalAmount} USD`, 287, 630, 150, {
        size: 10,
      });
...
const header = (pdf, invoice, pageNumber, hotel) => {
  ...
  pdf.addText('TOUR OPERATOR SALE INVOICE', 0, 50, 600, {
    size: 14,
    align: 'center',
  });
...
const footer = (pdf, invoice, offSet = 120) => {
  ...
  const hotelAmount = invoice.orderHotels.reduce(
    (sum, orderHotel) => sum + orderHotel.amount,
    0,
  );
  ...
  pdf.addText(`${invoice.amount}`, 470, 618 + offSet, 150, {
    size: 9,
  });
};
